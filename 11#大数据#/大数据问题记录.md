...
>
>   <font color="red">注意：添加/修改/删除lib目录下的jar，需要重启HS2才可以。</font>
>
...
Q. 自定义的UDF函数导入后，执行报错Error: Error while compiling statement: FAILED: SemanticException [Error 10014]: Line 1:7 Wrong arguments '"default.test01"': org.apache.hadoop.hive.ql.metadata.HiveException: Unable to execute method public java.lang.String com.sjl.hive.udf.NewestPartition.evaluate(java.lang.String) throws org.apache.thrift.TException with arguments {default.test01}:org.apache.hadoop.hive.metastore.HiveMetaStoreClient.<init>(Lorg/apache/hadoop/hive/conf/HiveConf;)V (state=42000,code=10014)

**答**：

> 问题现象：使用`create function f1 as "xxx" using jar "hdfs:///path/to/jar"`成功创建自定义的UDF函数，但是在使用`select f1("参数")`时报错。如下所示：
>
> ```sql
> 0: jdbc:hive2://hadoop322-node01:2181,hadoop3> select getnewestpartition("default.test01");
> Error: Error while compiling statement: FAILED: SemanticException [Error 10014]: Line 1:7 Wrong arguments '"default.test01"': org.apache.hadoop.hive.ql.metadata.HiveException: Unable to execute method public java.lang.String com.sjl.hive.udf.NewestPartition.evaluate(java.lang.String) throws org.apache.thrift.TException with arguments {default.test01}:org.apache.hadoop.hive.metastore.HiveMetaStoreClient.<init>(Lorg/apache/hadoop/hive/conf/HiveConf;)V (state=42000,code=10014)
> ```
>
> 查看hiveserver2的日志也没有任何收获。看报错提示是参数不对，但是实际在本地测试过，参数是没有问题的。
>
> 问题原因：TODO
>
> 解决方法：使用spark-sql同样的方式来调用此函数时是没有问题的。先用spark-sql吧。TODO
...
**答**：参考：https://www.cnblogs.com/ExMan/p/14358321.html https://www.codenong.com/cs106200265/
...
Q. 执行`spark-sql --master yarn`报错,Failed to load org.apache.spark.sql.hive.thriftserver.SparkSQLCLIDriver.
...
> spark3.0.2
>
> 问题原因：因为spark3.0.2默认兼容的是hvie2，因此在部署hive on spark的时候，对spark3.0.2的源码进行了重新编译，其中不包含任何hive的jar包。根据提示，不能加载 org.apache.spark.sql.hive.thriftserver.SparkSQLCLIDriver 这个类，而此报错就是因为在$SPARK_HOME/jars目录下没有找到spark-hive-thriftserver_xx_xx.jar。
>
> 解决方法：
>
> 1、使用如下命令重新编译spark3.0.2的源码。
>
> ```bash
> ./dev/make-distribution.sh --name "spark3.0.2-hadoop3.2.2-hive3.1.2" --tgz "-Pyarn,hadoop-3.2.2,parquet-provided,orc-provided,hive-thriftserver,hive" -DskipTests clean package  -X -e
> ```
>
> 2、解压编译好的zip包
>
> 3、修改`/etc/profile`或者是`/etc/profile.d`下你配置的SPARK_HOME环境变量，指向新的spark3.0.2的目录。例如
>
> ```bash
> vim /etc/profile.d/hadoop322_env.sh
> ```
>
> ​	但是，**修改了$SPARK_HOME之后不会导致hive on spark 会报错**。
>
> 4、将`$HIVE_HOME/conf/hive-site.xml`创建一个软连接到`$SPARK_HOME/conf`
>
> ```bash
> ln -s $HIVE_HOME/conf/hive-site.xml $SPARK_HOME/conf/hive-site.xml
> ```

Q. spark sql on hive 创建了UDF函数，但是执行函数报错：org.apache.spark.sql.execution.QueryExecutionException: java.net.UnknownHostException: user

**答**：

> 问题原因：创建UDF函数的语句时，自定义的UDF jar的位置写的不对，错误的如下（**hdfs://user/hive/udf-jars**)：
>
> ```bash
> create function helloworld as "com.sjl.hive.udf.HelloUDF" using jar "hdfs://user/hive/udf-jars/MyHiveUDF-1.1-SNAPSHOT.jar";
> 
> create function getnewpartition as "com.sjl.hive.udf.NewestPartition" using jar "hdfs://user/hive/udf-jars/MyHiveUDF-1.1-SNAPSHOT.jar";
> ```
>
> 导致spark sql无法获取到自定义的UDF jar。
>
> 解决方法：删除UDF函数，重新创建.
>
> （1）删除UDF函数
>
> ```sql
> drop function default.helloworld;
> drop function default.getnewpartition;
> ```
>
> （2）创建UDF函数
>
...
> （3）验证
>
> ![image-20230621120143295](大数据问题记录.assets/image-20230621120143295.png)

Q. spark sql中创建的udf函数，在hive beeline中却无法查看到？
...
>
> ```bash
> # 1、将自定义的udf jar包放到hdfs上
> hadoop fs -put MyHiveUDF-1.1-SNAPSHOT.jar /user/hive/udf-jars/
> 
> # 2、spark sql创建永久udf函数
...
> 
> # 4、使用函数
> > select helloworld("世界");
> ```
>
> 但是，在使用beeline连接到hiveserver2之后却看不见新建的这两个自定义UDF函数。
>
> ```bash
> bin/beeline -u "jdbc:hive2://hadoop322-node01:2181,hadoop322-node02:2181,hadoop322-node03:2181/;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2_zk" -n hadoop ""
> 
> # 发现无法查看到新建的两个永久自定义UDF函数。
> > show functions;
> ```
>
> 问题原因：
>
> 使用`create function helloworld as "com.sjl.hive.udf.HelloUDF" using jar "hdfs:///user/hive/udf-jars/MyHiveUDF-1.1-SNAPSHOT.jar";`方式创建UDF函数在hive的元数据库中的`metastore.FUNC_RU`表中记录了UDF jar的path，如下所示：
>
> ![image-20230621150401042](大数据问题记录.assets/image-20230621150401042.png)
>
> 但是由于这些jar是在hdfs上，那sparkSQL/hive肯定是无法加载的，所以会将这些jar，复制到本地的`/tmp`下，然后将这些jar加入到hive的classpath，这样就能获取到jar对应的UDF函数了。
>
> 而hiveserver2是每次重启的时候才会去重新加载这些jar，以及metastore元数据的变化，因为上面的创建永久UDF函数都没有经过hiveserver2，而是sparkSQL与hive metastore完成的，因此hiveserver2根本不知道元数据发生了变化（增加了两个UDF函数）。
>
> **结论**
>
> **在sparkSQL中创建的永久UDF函数，在hiveserver2中看不见，需要重启hiveserver2（重新加载元数据）才能看见。**
>
> **相反，在hiveserver2中创建的永久UDF函数，在sparkSQL中立即就能看到；**
>
> 解决方法：重启hiveserver2。
...
> （1）创建快照
>
> 在CDH中，可以在CMS上直接进行制作快照和恢复快照。我们按照开源的Apache HBase来演示：
>
> ```bash
> # 1、创建快照
> snapshot '表名', '快照名'
...
> 在hive中创建外表，关联到hbase中的表

Q. HBase集成phoenix之后，发现在phoenix中建的表没有对应到hbase中的namespace

答：

> 默认情况下，phoenix中的schema和hbase中的namespace是没有对应的。
>
