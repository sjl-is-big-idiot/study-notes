...
**答**：`sticky bit`只对HDFS的目录生效，当某文件夹开启了`sticky bit`之后，即使其它用户对该文件夹具有`rwx`权限，也只有`superuser`，`文件夹owner`、`文件owner`才能删除/移动目录中的文件。

Q. Hadoop中的proxyuser是干啥的？

**答**：关于hadoop的用户代理介绍：https://www.cnblogs.com/skyrim/p/14268192.html，官方文档：https://hadoop.apache.org/docs/stable/hadoop-project-dist/hadoop-common/Superusers.html
...
Q. YARN的timelineserver和MapReduce的JobHistoryServer区别？
...
> 所简单点就是，Hadoop1.x中没有Timeline Server，之后MR的JobHistoryServer，只支持MR类作业的信息的存储和检索。
>
